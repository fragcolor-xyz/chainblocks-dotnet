...
namespace Fragcolor.Shards.Claymore
{
  ...
  {
    private const string Dll = "claylib";
    ...
    /// </summary>
    /// <param name="wire">A reference to the wire from the <see cref="ClGetDataRequest"/>.</param>
    ...
    [DllImport(Dll, CallingConvention = Conv)]
    internal static extern SHBool clmrPoll(SHWireRef wire, out ClPollStatePtr state);

    /// <summary>
    /// Frees the state previously retrieved with <see cref="clmrPoll(SHWireRef, out ClPollStatePtr)"/>.
    /// </summary>
    /// <param name="state">A pointer to the state.</param>
    [DllImport(Dll, CallingConvention = Conv)]
    ...
    [DllImport(Dll, CallingConvention = Conv)]
    internal static extern ClUploadRequestPtr clmrUpload(ref SHVar var);
